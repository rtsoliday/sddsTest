...
int	new_m, new_n;
{
   ...
   {	/* re-allocate A->me */
	 ...
	 mem_bytes(TYPE_MAT,A->max_m*sizeof(Real *),
      ...
      {
	Real **tmp = A->me ? realloc(A->me, new_m * sizeof(Real*)) : calloc(new_m, sizeof(Real*));
	A->me = tmp;
      }
      if ( ! A->me )
   ...
   {	/* re-allocate A->base */
	 ...
	 mem_bytes(TYPE_MAT,A->max_m*A->max_n*sizeof(Real),
      ...
      {
	Real *tmp = A->base ? realloc(A->base, new_size * sizeof(Real)) : calloc(new_size, sizeof(Real));
	A->base = tmp;
      }
      if ( ! A->base )
	error(E_MEM,"m_resize");
   ...
   if ( A->max_n < new_n )
   {
	 ...
	 {
	   Real *tmp = A->me[i] ? realloc(A->me[i], new_max_n * sizeof(Real)) : calloc(new_max_n, sizeof(Realx));
	   A->me[i] = tmp;
	 }
	 if ( ! A->me[i] )
	   error(E_MEM,"m_resize");
...
int	new_size;
{
   ...
   if ( new_size > px->max_size )
   {
	 ...
	 mem_bytes(TYPE_PERM,px->max_size*sizeof(unsigned int),
      ...
      {
	unsigned int *tmp = px->pe ? realloc(px->pe, new_size * sizeof(unsigned int)) : calloc(new_size, sizeof(unsigned int));
	px->pe = tmp;
      }
      if ( ! px->pe )
...
int	new_dim;
{
   ...
   if ( new_dim > x->max_dim )
   {
	 ...
	 mem_bytes(TYPE_VEC,x->max_dim*sizeof(Real),
      ...
      {
	Real *tmp = x->ve ? realloc(x->ve, new_dim * sizeof(Real)) : calloc(new_dim, sizeof(Real));
	x->ve = tmp;
      }
      if ( ! x->ve )
