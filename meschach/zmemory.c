...
int	new_m, new_n;
{
   ...
   {	/* re-allocate A->me */
	 ...
	 mem_bytes(TYPE_ZMAT,A->max_m*sizeof(complex *),
      ...
      {
	complex **tmp = A->me ? realloc(A->me, new_m * sizeof(complex*)) : calloc(new_m, sizeof(complex*));
	A->me = tmp;
      }
      if ( ! A->me )
   ...
   {	/* re-allocate A->base */
	 ...
	 mem_bytes(TYPE_ZMAT,A->max_m*A->max_n*sizeof(complex),
      ...
      {
	complex *tmp = A->base ? realloc(A->base, new_size * sizeof(complex)) : calloc(new_size, sizeof(complex));
	A->base = tmp;
      }
      if ( ! A->base )
	error(E_MEM,"zm_resize");
   ...
   if ( A->max_n < new_n )
   {
	 ...
	 {
	   complex *tmp = A->me[i] ? realloc(A->me[i], new_max_n * sizeof(complex)) : calloc(new_max_n, sizeof(complex));
	   A->me[i] = tmp;
	 }
	 if ( ! A->me[i] )
	   error(E_MEM,"zm_resize");
...
int	new_dim;
{
   ...
   if ( new_dim > x->max_dim )
   {
	 ...
	 mem_bytes(TYPE_ZVEC,x->max_dim*sizeof(complex),
      ...
      {
	complex *tmp = x->ve ? realloc(x->ve, new_dim * sizeof(complex)) : calloc(new_dim, sizeof(complex));
	x->ve = tmp;
      }
      if ( ! x->ve )
