		...
		size_t *restrict out_pos, size_t out_size, lzma_action action)
{
	...
	case SEQ_STREAM_FOOTER:
		...
		coder->buffer_pos = 0;
		if (coder->sequence == SEQ_STREAM_HEADER)
		  coder->sequence = SEQ_BLOCK_INIT;
		else
		  coder->sequence = SEQ_BLOCK_ENCODE;
		break;
		...
		const lzma_filter *filters, lzma_check check)
{
	...
	if (next->coder == NULL) {
	  next->coder = (lzma_coder*)lzma_alloc(sizeof(lzma_coder), allocator);
		...
		lzma_next_coder_s tmp1 = LZMA_NEXT_CODER_INIT;
		next->coder->block_encoder = tmp1;
		lzma_next_coder_s tmp2 = LZMA_NEXT_CODER_INIT;
		next->coder->index_encoder = tmp2;
#else
