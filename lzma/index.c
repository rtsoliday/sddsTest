...
static void
index_cat_helper(const index_cat_info *info, index_stream *thiss)
{
	index_stream *left = (index_stream *)(thiss->node.left);
	index_stream *right = (index_stream *)(thiss->node.right);

	if (left != NULL)
		index_cat_helper(info, left);

	thiss->node.uncompressed_base += info->uncompressed_size;
	thiss->node.compressed_base += info->file_size;
	thiss->number += info->stream_number_add;
	thiss->block_number_base += info->block_number_add;
	index_tree_append(info->streams, &thiss->node);
...
index_dup_stream(const index_stream *src, lzma_allocator *allocator)
{
	...
	do {
		...
		i += srcg->last + 1;
		srcg = (const index_group *)index_tree_next(&srcg->node);
	} while (srcg != NULL);
...
lzma_index_dup(const lzma_index *src, lzma_allocator *allocator)
{
	...
	do {
		...
		srcstream = (const index_stream*)index_tree_next(&srcstream->node);
...
{
  const lzma_index *i = (const lzma_index*)iter->internal[ITER_INDEX].p;
  const index_stream *stream = (const index_stream*)iter->internal[ITER_STREAM].p;
  const index_group *group = (const index_group*)iter->internal[ITER_GROUP].p;
...
lzma_index_iter_next(lzma_index_iter *iter, lzma_index_iter_mode mode)
{
	...
	if (mode != LZMA_INDEX_ITER_STREAM) {
		...
		case ITER_METHOD_NORMAL:
		  group = (const index_group*)iter->internal[ITER_GROUP].p;
			break;

		case ITER_METHOD_NEXT:
		  group = (const index_group*)index_tree_next((const index_tree_node*)iter->internal[ITER_GROUP].p);
			break;
	...
	if (stream == NULL) {
		...
		if (mode >= LZMA_INDEX_ITER_BLOCK) {
			...
			while (stream->groups.leftmost == NULL) {
			  stream = (const index_stream*)index_tree_next(&stream->node);
				if (stream == NULL)
					return true;
			}
		}
		...
		if (group != NULL)
		  group = (const index_group*)index_tree_next(&group->node);
...
lzma_index_iter_locate(lzma_index_iter *iter, lzma_vli target)
{
  const lzma_index *i = (const lzma_index*)iter->internal[ITER_INDEX].p;
	...
	const index_stream *stream = (const index_stream*)index_tree_locate(&i->streams, target);
	assert(stream != NULL);
	target -= stream->node.uncompressed_base;

	// Locate the group containing the target offset.
	const index_group *group = (const index_group*)index_tree_locate(&stream->groups, target);
	assert(group != NULL);
